...
class ColbertSymMarginPairwiseCELoss(ColbertModule):
    def __init__(
        self,
        temperature: float = 1.0,
        weight_main: float = 1,
        weight_pos: float = 0.1,  
        weight_neg: float = 0.1,
        weight_query_reg: float = 0.1,
        normalize_scores: bool = True,
        use_smooth_max: bool = False,
        pos_aware_negative_filtering: bool = False,
        max_batch_size: int = 1024,
        ...
        self.weight_query_reg = weight_query_reg
        self.normalize_scores = normalize_scores
        self.use_smooth_max = use_smooth_max
        self.pos_aware_negative_filtering = pos_aware_negative_filtering

    def compute_query_regularization(self, query_embeddings: torch.Tensor) -> torch.Tensor:
        # query_embeddings: [B, Q_len, Dim]
        q_norm = F.normalize(query_embeddings, p=2, dim=-1)
        sim_matrix = torch.bmm(q_norm, q_norm.transpose(1, 2))
        
        # (a) Padding Mask: 
        # [B, Q] -> [B, Q, 1] * [B, 1, Q] -> [B, Q, Q]
        mask_len = (query_embeddings.abs().sum(dim=-1) > 1e-9).float()
        valid_mask = torch.bmm(mask_len.unsqueeze(2), mask_len.unsqueeze(1))
        
        # (b) Diagonal Mask
        batch_size, seq_len, _ = query_embeddings.shape
        eye = torch.eye(seq_len, device=query_embeddings.device).unsqueeze(0)
        
        off_diag_sim = sim_matrix * (1 - eye) * valid_mask
        loss_query = (off_diag_sim ** 2).sum() / (valid_mask.sum() - batch_size * seq_len + 1e-9)
        
        return loss_query

    def compute_length_regularization(
        self, 
        interaction_matrix: torch.Tensor, 
        pos_idx: torch.Tensor, 
        neg_idx: torch.Tensor
    ) -> torch.Tensor:
        cummax_vals, _ = interaction_matrix.cummax(dim=3)
        cumulative_scores = cummax_vals.sum(dim=2)
        
        delta = torch.zeros_like(cumulative_scores)
        delta[..., 0] = cumulative_scores[..., 0]
        delta[..., 1:] = cumulative_scores[..., 1:] - cumulative_scores[..., :-1]
        
        batch_size = delta.size(0)
        idx = self.idx_buffer[:batch_size].to(delta.device)

        pos_delta = delta[idx, pos_idx, :]
        neg_delta = delta[idx, neg_idx, :]
        
        loss_pos = -pos_delta.mean()
        loss_neg = neg_delta.mean()
        
        return loss_pos, loss_neg

    def forward(self, query_embeddings: torch.Tensor, doc_embeddings: torch.Tensor, offset: int = 0) -> torch.Tensor:
        interaction_matrix = torch.einsum("bnd,csd->bcns", query_embeddings, doc_embeddings)
        raw_scores = self._aggregate(interaction_matrix, self.use_smooth_max, dim_max=3, dim_sum=2)
        
        lengths = (query_embeddings[:, :, 0] != 0).sum(dim=1)
        if self.normalize_scores:
            scores = self._apply_normalization(raw_scores, lengths)
        else:
            scores = raw_scores
        ...
        pos_scores = scores[idx, pos_idx]
        top2_vals, top2_indices = scores.topk(2, dim=1)
        is_pos_top1 = (top2_indices[:, 0] == pos_idx)
        neg_scores = torch.where(is_pos_top1, top2_vals[:, 1], top2_vals[:, 0])
        
        hard_neg_idx = torch.where(is_pos_top1, top2_indices[:, 1], top2_indices[:, 0])
        main_loss = F.softplus((neg_scores - pos_scores) / self.temperature).mean()
        
        total_loss = self.weight_main * main_loss

        if self.weight_pos > 0 or self.weight_neg > 0:
            loss_pos, loss_neg = self.compute_length_regularization(interaction_matrix, pos_idx, hard_neg_idx)
            total_loss += self.weight_pos * loss_pos + self.weight_neg * loss_neg

        if self.weight_query_reg > 0:
            loss_query = self.compute_query_regularization(query_embeddings)
            total_loss += self.weight_query_reg * loss_query
            if "0" in str(main_loss.device) and torch.rand(1).item() < 0.01:
                 print(f"Losses -> Main: {main_loss.item():.4f}, QueryReg: {loss_query.item():.4f}")
