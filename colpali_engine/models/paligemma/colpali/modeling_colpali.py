...
class CausalPali(PaliGemmaPreTrainedModel):
    """
    CausalPali model implementation, following the architecture ideas from CausalQwen
    but without the projection layer and using autoregressive latent tokens.

    Args:
        config (PaliGemmaConfig): The model configuration.
        doc_token_num (int): Number of latent tokens to generate for documents.
        query_token_num (int): Number of latent tokens to generate for queries.
        mask_non_image_embeddings (Optional[bool]): Unused in this architecture but kept for API compatibility.
    """
    ...
    def __init__(
        ...
        doc_token_num: int = 32, 
        query_token_num: int = 16, 
        ...
        self.latent_token_id = getattr(self.config.text_config, "eos_token_id", None)
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        last_hidden_states = outputs.hidden_states[-1]  # (batch_size, sequence_length, hidden_size)
        # print(len(outputs.hidden_states), last_hidden_states.shape)
        
        # Start the latent sequence with the last token of the input
        latent_token_embeds = [last_hidden_states[:, -1:, :]]

        batch_size = last_hidden_states.shape[0]
        
        if self.latent_token_id is None:
            raise ValueError("`latent_token_id` is None. Please set it (e.g. to model.config.text_config.eos_token_id) before calling forward.")

        # Prepare input for autoregressive generation
        ...
        # 2. Autoregressive Generation of Latent Tokens
        # Loop num_latent_tokens - 1 times (since we already have the first one)
        for i in range(num_latent_tokens - 1):
            outputs = self.model(
                ...
                use_cache=True,
                output_hidden_states=True,
                # Note: We do not pass pixel_values here; they are cached in past_key_values
            )
            
            past_key_values = outputs.past_key_values
            last_hidden_states = outputs.hidden_states[-1]  # (batch_size, 1, hidden_size)
            # print(len(outputs.hidden_states),outputs.hidden_states[0].shape,outputs.hidden_states[1].shape, last_hidden_states.shape)
            # print(past_key_values.layers[0].keys.shape,past_key_values.layers[0].values.shape)
            latent_token_embeds.append(last_hidden_states)

        # 3. Concatenate and Normalize
        # (batch_size, num_latent_tokens, hidden_size)
        proj = torch.cat(latent_token_embeds, dim=1) 
        
        # L2 normalization
        proj = proj / proj.norm(dim=-1, keepdim=True)  # (batch_size, num_latent_tokens, hidden_size)
